<template>
    <div>
        <breadcrumb active_name="Daily Sales" :links="[{name:'Home',url:'/'},{name:'Products',url:'/products'}]"></breadcrumb>
...
export default {
    ...
    methods:{
      ...
      async  getOrders(page=1){
        this.page_num=page;
        this.loading=true;
        const url=`/report/daily?page=${this.page_num}&query=${this.query}`;
         await axios.get(url).then((res)=>{
                this.orders=res.data.daily_report;
